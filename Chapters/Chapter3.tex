...
Antes de configurarse en modo ahorro de energía, se habilita la interrupción por flanco ascendente en el pin físico 10. La señal conectada a este pin, es la salida del circuito de la figura \ref{fig:ctodetectorcortes}.
La funcion asociada a la interrupción, genera una señal de restablecimiento del sistema y el ciclo de la figura \ref{fig:patronpowersaveloop} se repite.\\
En caso de que no exista una interrupción, el microcontrolador permanecerá en modo ahorro de energía durante 5 minutos y se restablecerá debido al \textit{timeout} generado.\\
...
\textit{The Things Network} provee una herramienta dedicada llamada \textit{payload format decoder}. Con esta herramienta cada vez que un paquete de un nodo llega, el decodificador lo procesa y separa cada lectura como clave de un elemento JSON (figura \ref{fig:payloaddecoder}) antes de realizar el env\'{i}o a los servicios de backend.\\
% TODO: \usepackage{graphicx} required
\begin{figure}[h]
	\centering
